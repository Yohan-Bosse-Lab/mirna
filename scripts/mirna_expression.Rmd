...
#check what? 
```{r data}
files = list.files(file.path(params$resultspath,'bams'),recursive = TRUE,pattern = 'idxstats$')

for(i in 1:length(files)){
  
  temp = read.table(file.path(params$resultspath,'bams',files[i]),header =F)
  temp = temp[-nrow(temp),]
...
```{r session, message= T}
#save it 
pdf(file.path(params$figurespath,paste0('Figure1_miRNA.pdf')),width = 12,height = 8)
